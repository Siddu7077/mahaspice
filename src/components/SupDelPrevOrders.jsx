...
const DeliveryPrevOrders = ({ activeTab }) => {
    ...
    const fetchOrders = async (phone, type) => {
        setLoading(true);
        try {
            const response = await fetch(`https://adminmahaspice.in/ms3/SupDelPrevOrders.php?phone=${phone}&type=${type}`);
            ...
            {orders.map((order) => (
                <div key={order.payment_order.payment_id} className="bg-white rounded-lg shadow-md p-6 mb-6">
                    ...
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <span
                            className={`px-3 py-1 rounded-full text-sm ${order.payment_order.status === "completed"
