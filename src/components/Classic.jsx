...
const Classic = () => {
  ...
  const calculatePlatePrice = () => {
    ...
    const classicPricing = pricingData.find(
      price => price.crpb_name.toLowerCase() === 'classic'
    );

    if (!classicPricing) return 0;

    const basePrice = menuPreference === "veg" 
      ? parseFloat(classicPricing.veg_price)
      : parseFloat(classicPricing.non_veg_price);
