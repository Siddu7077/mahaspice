  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (itemsResponse.data.success && categoriesResponse.data.success) {
          // Filter items with type 'classic'
          const classicItems = itemsResponse.data.items.filter(item => {
            const category = categoriesResponse.data.categories.find(cat => cat.id === item.category_id);
            return category && category.type === 'classic';
          });

          const mappedItems = classicItems.map(item => ({
            ...item,
            id: item.id,
            name: item.item_name,
          ...
          const classicCategories = categoriesResponse.data.categories.filter(cat => cat.type === 'classic');
          const mappedCategories = classicCategories.map(cat => ({
            ...cat,
            name: cat.category,
            id: cat.id,
          ...
          if (pricingResponse.data) {
            const classicPricing = pricingResponse.data.find(item => item.name === "Classic");
            setPricingData(classicPricing);
          }
        }
  ...
  const renderPriceHeader = () => {
    ...
    return (
      <div className="flex items-center gap-2 bg-blue-50 px-4 py-2 rounded-lg">
        <h1 className="text-3xl font-bold text-gray-800">Classic Menu</h1>
